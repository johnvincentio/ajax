    ...
    buildButtons: function(data) {
        ...
        if (data.items && data.items.length > 0) {
            ...
            if (data.prevPageToken) {
                result += '<button class="button js-prev" type="button">&lt; Previous</button>';
            }
            if (data.nextPageToken) {
                result += '<button class="button js-next" type="button">Next &gt;</button>';
            }
            result += '</div>';
        }
        return result;
    },
    renderSearchContent: function(data, elementItems, elementButtons) {
...
//                           " item.snippet.channelId "+item.snippet.channelId);
        ...
        elementItems.html(html);
        elementButtons.html(this.buildButtons(data));
//        console.log("<<< renderSearchContent");
    }
};
